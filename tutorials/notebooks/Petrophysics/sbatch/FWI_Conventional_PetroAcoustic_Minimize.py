...
# ====== Histórico e log ======
history = []
with open("txt/fo_results_FWI_conventional.txt", "w") as f:
...
def cb(xk):
    ...
    with open("txt/fo_results_FWI_conventional.txt", "a") as f:
...
res_opt = minimize(
    ...
    callback=cb,
    options=dict(
        maxiter=iterations,   # defina 'iterations' antes ou troque por um número
        ftol=1e-12,
        maxcor=20,
        disp=True
    )
...
phi_inv, vsh_inv, sw_inv = unpack(res_opt.x)

# (opcional) garantir limites e travar water layer
...
np.savez_compressed("results/petro_inv_conventional.npz", phi=phi_inv, vsh=vsh_inv, sw=sw_inv)
...
plt.colorbar()
plt.tight_layout()
plt.savefig('images/conventional_minimize/results_FWI_Conventional.png', format='png', dpi=200)

# Plotando a função objetivo (FO) ao longo das iterações
...
plt.legend()
plt.tight_layout()
plt.savefig('images/conventional_minimize/FO_FWI_Conventional.png', format='png', dpi=200)
