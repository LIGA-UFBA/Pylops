...
if devito_message is None:
    ...
    from examples.seismic.acoustic import AcousticWaveSolver, AcousticModel
...
class _PetroAcousticWave(_Wave): #Petro
    ...
    def __init__(
        ...
        vsh: NDArray, #shale_volume
        sw: NDArray, #water_saturation
        src_x: NDArray,
        src_z: NDArray,
        rec_x: NDArray,
        rec_z: NDArray,
        ...
        # create model
        self._create_model(shape, origin, spacing, vp, phi, vsh, sw, space_order, nbl, dt, **model_kwargs)
        self._create_geometry(
            src_x, src_y, src_z, rec_x, rec_y, rec_z, t0, tn, src_type, f0=f0
        )
        self.checkpointing = checkpointing
        self.multi_input = multi_input #New
        self.karguments = {}
        self.karguments.update({
        "phi": self.model.phi,
        "vsh": self.model.vsh,
        "sw":  self.model.sw,
        **{k: getattr(self.model, k) for k in ("a1","a2","a3","a4") if hasattr(self.model,k)}})
        ...
        # in_ch = 4 if self.multi_input else 1      # New
        in_ch = 3 if self.multi_input else 1
        dims_in = (in_ch, *vp.shape) if self.multi_input else vp.shape #New
    ...
    def _create_model(
        ...
        self.model = AcousticModel(vp=vp*1e-3, phi=phi, vsh=vsh, sw=sw, origin=origin, 
                     space_order=space_order, shape=shape, dtype=np.float32, 
                              spacing=spacing, nbl=nbl, bcs='damp', dt=dt, **model_kwargs) #bcs="damp"
    ...
    def _bornadj_oneshot(self, solver: AcousticWaveSolver, isrc, dobs):
        ...
        model = solver.jacobian_adjoint(
            rec=recs, u=u0, checkpointing=self.checkpointing, **self.karguments
        )[0:3]
        return model
    ...
    def _bornadj_allshots(self, dobs: NDArray) -> NDArray:
        ...
        nxz = self.model.shape
        mtot = np.zeros((3, *nxz), dtype=np.float32)  # (phi, vsh, sw)
    
        nsrc = self.geometry.src_positions.shape[0]
        for isrc in range(nsrc):
            solver.geometry.src_positions = self.geometry.src_positions[isrc, :]
            gphi, gvsh, gsw = self._bornadj_oneshot(solver, isrc, dobs[isrc])
            mtot[0] += self._crop_model(gphi.data, self.model.nbl)
            mtot[1] += self._crop_model(gvsh.data, self.model.nbl)
            mtot[2] += self._crop_model(gsw.data,  self.model.nbl)
    
        # adiciona canal "vp" = zero só pra casar com dims=(4, nx, nz)
        # vp_zero = np.zeros(nxz, dtype=np.float32)
        # mtot4 = np.vstack([vp_zero[None, ...], mtot])   # (4, nx, nz)
        # return mtot4
        return mtot

    def _to_param_fn(self, name: str, arr: NDArray) -> Function:
        fn = Function(
            name=name,
            grid=self.model.grid,
        ...
        # preenche incluindo PML (igual ao resto do código)
        initialize_function(fn, arr, self.model.padsizes, mode="constant")
        return fn
    ...
    def _fwd_oneshot(self, isrc, solver: AcousticWaveSolver, v: NDArray) -> NDArray:
        ...
        if self.multi_input:
            # espera v com primeiro eixo = 3
            phi_arr, vsh_arr, sw_arr = v
            phi_fn = self._to_param_fn("phi", phi_arr)
            vsh_fn = self._to_param_fn("vsh", vsh_arr)
            sw_fn  = self._to_param_fn("sw",  sw_arr)

        self.karguments.update({"phi": phi_fn, "vsh": vsh_fn, "sw": sw_fn})
...
class _ViscoAcousticWave(_Wave):
    ...
    def __init__(
        ...
        self.karguments = {}
        # self.karguments.update({"vp": self.model.vp, "b": self.model.b, "qp": self.model.qp}
        # self.karguments.update({"rho": 1./self.model.b})
        self.op_name = op_name
    ...
    def _fwd_oneshot(self, solver: ViscoacousticWaveSolver, v: NDArray) -> NDArray:
        # v esperado com shape (2, nx, nz)
        vp_arr, rho_arr = v
        ...
        # preenche com pads
        initialize_function(vp_fn,  vp_arr,  self.model.padsizes)
        # initialize_function(rho_fn, rho_arr, self.model.padsizes)
        initialize_function(b_fn,   1.0 / rho_arr, self.model.padsizes)
    
        # passa para o solver
        # self.karguments.update({"vp": vp_fn, "rho": rho_fn, "b": b_fn})
        self.karguments.update({"vp": vp_fn, "b": b_fn})
    ...
    def _grad_oneshot(self, solver: ViscoacousticWaveSolver, isrc, dobs):
        ...
        # source wavefield
        if hasattr(self, "src_wavefield"):
            u0, v0 = self.src_wavefield[isrc]
        else:
            # u0 = solver.forward(save=True, src=None if not hasattr(self, "wav") else self.wav)[1]
            recs_fwd, u0, v0, _ = solver.forward(save=True, 
                                             src=None if not hasattr(self, "wav") else self.wav)
    ...
    def _grad_allshots(self, dobs: NDArray) -> NDArray:
        ...
        nxz = self.model.shape
        mtot = np.zeros((2, *nxz), dtype=np.float32)
        nsrc = self.geometry.src_positions.shape[0]
        for isrc in range(nsrc):
            solver.geometry.src_positions = self.geometry.src_positions[isrc, :]
            gvp, grho = self._grad_oneshot(solver, isrc, dobs[isrc])
            # print(gvp.data.max(), grho.data.max())
            mtot[0] += self._crop_model(gvp.data, self.model.nbl)
            mtot[1] += self._crop_model(grho.data, self.model.nbl)
        return mtot
    ...
    def adjoint(self):
        """
        Adapta o operador adjunto para ter 2 canais de saída (grad_vp e grad_rho)
        quando usado no modo 'fwd'.
        """
        if self.op_name == "fwd":
            # cria uma cópia para não mexer no operador original
            Op = deepcopy(self)
            # dims originais são (nx, nz) -> queremos (2, nx, nz)
            new_dims = (Op.dims) #(2, *Op.dims)
            Op._update_dimensions(new_dims, Op.dimsd)
            return LinearOperator.adjoint(Op)
        # comportamento padrão se for outro modo
        return LinearOperator.adjoint(self)

        # if self.op_name == "fwd":
        #     Op = deepcopy(self)
    
        #     # dims = dimensão de entrada do adjoint → é igual ao data space
        #     # dimsd = dimensão de saída do adjoint → deve ser (2, nx, nz)
        #     nx, nz = self.dims   # dims originais (nx, nz)
    
        #     new_dimsd = (2, nx, nz)
        #     Op._update_dimensions(new_dims=self.dims, new_dimsd=new_dimsd)

        #     return LinearOperator.adjoint(Op)
    
        # return LinearOperator.adjoint(self)

    # para usar Dop.H como atalho para o adjoint
...
PetroAcousticWave2D = _PetroAcousticWave
PetroAcousticWave3D = _PetroAcousticWave
